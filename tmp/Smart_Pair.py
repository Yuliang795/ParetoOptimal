import numpy as np
import sys
from cc_class import connected_component



class node_graph:
  def __init__(self, excl_encoder,df_len):
    self.cc_set = []
    self.E_plus = []
    self.E_minus = []
    self.excl_encoder = excl_encoder
    self.node_table = np.full((df_len,), np.nan)


  def get_cc_ind(self,p):
    p0_cc_ind, p1_cc_ind = self.node_table[p]
    if np.isnan(p0_cc_ind):
      p0_cc_ind = None
    else:
      p0_cc_ind = int(p0_cc_ind)
    if  np.isnan(p1_cc_ind):
      p1_cc_ind = None
    else:
      p1_cc_ind = int(p1_cc_ind)
    
    return p0_cc_ind, p1_cc_ind
  # 
  def update_node_table(self, rm_cc_ind):
    self.node_table[self.node_table>rm_cc_ind]-=1
  # update by adding a new point from E+
  # the new pair CANNOT be innor
  def update_cc_set(self,p, p0_cc_ind,p1_cc_ind):
    # p0_cc_ind,p1_cc_ind=None,None
    # for cc_ind, cc in enumerate(self.cc_set):
    #   if p[0] in cc.get_cc_point_list():
    #     p0_cc_ind = cc_ind
    #   if p[1] in cc.get_cc_point_list():
    #     p1_cc_ind = cc_ind

    # if p0 and p1 both are connected to a component set, merge these two component sets
    if p0_cc_ind !=None and p1_cc_ind !=None:
      cc_ind_small, cc_ind_large = sorted([p0_cc_ind, p1_cc_ind])
      # hash table
      self.node_table[self.cc_set[cc_ind_large].get_cc_point_list()] = cc_ind_small
      self.update_node_table(rm_cc_ind = cc_ind_large)
      ## neg edge table
      # self.neg_edge_table.merge_neg_edge(p0_cc_ind,p1_cc_ind)
      #
      self.cc_set[cc_ind_small].merge_cc(self.cc_set[cc_ind_large])
      self.cc_set.pop(cc_ind_large)
      return True

    # if only p0 connected, add the other point to the set where p0 exists
    if p0_cc_ind != None:
      # hash table
      self.node_table[p[1]] = p0_cc_ind
      #
      self.cc_set[p0_cc_ind].update_cc(p[1])
      return True
    # if only p1 exists in a set, add the other point to the set where p1 exists
    if p1_cc_ind !=None:
      # hash table
      self.node_table[p[0]] = p1_cc_ind
      #
      self.cc_set[p1_cc_ind].update_cc(p[0])
      return True
    # if none of p0 or p1 is in the set, then add a new connected component set for these two points
    self.cc_set.append(connected_component(p))
    self.node_table[p] = len(self.cc_set)-1

  # input is a pair of points
  def check_inner(self, p0_cc_ind,p1_cc_ind):
    # check only if the connected components set non-empty
    if self.cc_set:
      if p0_cc_ind==p1_cc_ind==None:
        return False
      if p0_cc_ind==p1_cc_ind:
        return True
    return False

    # if self.cc_set:
    #   for cc_ind, cc in enumerate(self.cc_set):
    #     if p[0] in cc.get_cc_point_list() and p[1] in cc.get_cc_point_list():
    #       return True
    #   # if they are not in the same cc, return False
    #   return False
    # # if the connected components set is empty, return False
    # else:
    #   return False

  # check crossing
  # the input is a pair of points
  def check_crossing(self, p0_cc_ind,p1_cc_ind):
    # if self.cc_set:
    #   p0_cc_ind,p1_cc_ind=None,None
    #   for cc_ind, cc in enumerate(self.cc_set):
    #     if p[0] in cc.get_cc_point_list():
    #       p0_cc_ind = cc_ind
    #     if p[1] in cc.get_cc_point_list():
    #       p1_cc_ind = cc_ind

        # print(f"{cc_ind} -- {cc.get_cc_point_list()}")
    # if self.cc_set:
    # both of the point have to be in a set, and the sets has to be different
    if p0_cc_ind!=None and p1_cc_ind!=None:
      # print(f"both not non  {p0_cc_ind} - {p1_cc_ind}   ---p {p}")
      # check for different cc set, the two points have to come from two different cc set
      if p0_cc_ind!=p1_cc_ind:
        # check if their cc sets is mutually exclusive
        # if self.cc_set[p0_cc_ind].check_excl(self.cc_set[p1_cc_ind]):
        if self.cc_set[p0_cc_ind].check_excl(self.cc_set[p1_cc_ind]):
        # if intersect(self.cc_set[p0_cc_ind].get_excl_cc_set(), self.cc_set[p1_cc_ind].get_excl_cc_set()):
            return True
    # if the cc set is empty, or the two point not in two different set, return False
    return False




  # Note: updating the pos edge set will also update the set of connected components
  def update_pos_edge(self, p, p0_cc_ind,p1_cc_ind):
    self.E_plus.append(p)
    # update the cc list after adding new point
    self.update_cc_set(p, p0_cc_ind,p1_cc_ind)



  def update_excl_cc(self, p,p0_cc_ind,p1_cc_ind):

    # p0_cc_ind,p1_cc_ind=None,None
    # for cc_ind, cc in enumerate(self.cc_set):
    #   if p[0] in cc.get_cc_point_list():
    #     p0_cc_ind = cc_ind
    #   if p[1] in cc.get_cc_point_list():
    #     p1_cc_ind = cc_ind

    # both of the point are in a different set
    # mark the two set as exclusive by adding a unique encoding
    if p0_cc_ind!=None and p1_cc_ind!=None:
      #
        unique_encoding = next(self.excl_encoder)
        try:
          self.cc_set[p0_cc_ind].add_excl_enc(unique_encoding)
          self.cc_set[p1_cc_ind].add_excl_enc(unique_encoding)
        except:
          print(f'***** p0_cc_ind:{p0_cc_ind}, p1:{p1_cc_ind}, ccset:{len(self.cc_set)}')
          sys.exit()

        ## neg edge table
        # self.neg_edge_table.insert_edge(p0_cc_ind,p1_cc_ind)
        
        return True

    # both of them is new
    # create two new cc sets for each of them
    # and mark the two sets as exclusive
    if p0_cc_ind==None and p1_cc_ind==None:
        unique_encoding = next(self.excl_encoder)
        self.cc_set.append(connected_component([p[0]], unique_encoding))
        self.cc_set.append(connected_component([p[1]], unique_encoding))
        # hash table
        self.node_table[p[0]] = len(self.cc_set)-2
        self.node_table[p[1]] = len(self.cc_set)-1

        ## neg edge table
        # self.neg_edge_table.insert_edge(len(self.cc_set)-2, len(self.cc_set)-1)
        return True

    # only one of the point already exists in one set and the other one isn't
    # add the unique encoding to the set where one of the points exists
    # create a new cc for the other one, and add the unique encoding
    if p0_cc_ind!=None:
        # (1)
        unique_encoding = next(self.excl_encoder)
        self.cc_set[p0_cc_ind].add_excl_enc(unique_encoding)
        # (2)
        self.cc_set.append(connected_component([p[1]], unique_encoding))
        # hash table
        self.node_table[p[1]] = len(self.cc_set)-1

        ## neg edge table
        # self.neg_edge_table.insert_edge(p0_cc_ind,len(self.cc_set)-1)
        return True

    if p1_cc_ind!=None:
        # (1)
        unique_encoding = next(self.excl_encoder)
        self.cc_set[p1_cc_ind].add_excl_enc(unique_encoding)
        # (2)
        self.cc_set.append(connected_component([p[0]], unique_encoding))
        # hash table
        self.node_table[p[0]] = len(self.cc_set)-1
        ## neg edge table
        # self.neg_edge_table.insert_edge(len(self.cc_set)-1,p1_cc_ind)
        return True

    # # if the cc set is empty, return False
    # return False

  # update the neg edge and update the exclusive cc set
  def update_neg_edge(self, p, p0_cc_ind,p1_cc_ind):
    self.E_minus.append(p)
    self.update_excl_cc(p, p0_cc_ind,p1_cc_ind)

  # print the cc point list of each cc
  def print_cc(self):
    for cc_ind, cc in enumerate(self.cc_set):
      print(f"connected component {cc_ind}")
      print(cc.get_cc_point_list())
      print("---exclusive list")
      # print(cc.excl_cc_set)
      print("\n")
















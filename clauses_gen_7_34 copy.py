	...
	if use_SmartPair:
		...
		while not infeasible_flag:
			...
			for pair in CL_descending:
				p0_cc_ind,p1_cc_ind = t1.get_cc_ind(pair)
				if t1.check_inner(p0_cc_ind,p1_cc_ind):
					print("------infeasible")
					infeasible_flag = True
					break
				if not t1.check_crossing(p0_cc_ind,p1_cc_ind):
					t1.update_neg_edge(pair,p0_cc_ind,p1_cc_ind)

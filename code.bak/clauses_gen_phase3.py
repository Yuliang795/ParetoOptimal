...
HARD_CLAUSE_W = SOFT_CLAUSE_W_b1*NUM_DISTANCE_CLASS + SOFT_CLAUSE_W_b0*NUM_DISTANCE_CLASS + 1
...
# for iter_ind in range(3):
    outfile_handle = open(f'{tmp_solution_path}p3_out_{iter_ind}', 'w')
    sys.stdout=outfile_handle
    
    curr_b0_sat =np.sum(b0_res)
    print(f'---curr_b1_sat {curr_b0_sat} - ')
    if curr_b0_sat>=len(b1):
        break
    
    # NUM_b1 = NUM_DISTANCE_CLASS - curr_b1_sat
    
    ### Start file I/O
    clause_file_name = 'p3_clauses_' + consts_path.split('/')[-1]
    try:
    ...
    print(f'========== b0  {curr_b0_sat} --- {b0[curr_b0_sat]}')
    if iter_ind==0:
        np.savetxt(f,np.array([HARD_CLAUSE_W,-b0[curr_b0_sat],0]),fmt='%d')
    ...
    Final_Clauses_File_Name = tmp_solution_path + f'phase_3_clauses_final_iter{iter_ind}'
    ...
    # loandra solver
    loandra_res_file_name = tmp_solution_path + f'phase_2_loandra_res_iter{iter_ind}'  #f'{data_file_path.split("_")[-1]}_loandra_res'
